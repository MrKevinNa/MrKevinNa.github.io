...
testSet$pred <- knn(train = trainSet[, 1:11], 
                    test = testSet[, 1:11], 
                    cl = trainSet$grade, 
...
testSet$predBal <- knn(train = trainSetBal[, 1:11], 
                       test = testSet[, 1:11], 
...
가중치 있는 KNN 분류모형은 **kknn** 패키지의 `kknn()` 함수를 이용하여 적합할 수 있습니다. 주요 인자는 다음과 같습니다.  
  ...
  - `train` : 훈련용 데이터셋의 입력변수들만 할당합니다.  
  - `test` : 시험용 데이터셋의 입력변수들만 할당합니다.  
...
```{r}
# 유클리디안 거리로 가중치를 준 knn 분류모형을 적합합니다. 
fitKnnW <- kknn(formula = grade ~ ., 
                train = trainSetBal[, 1:11], 
                test = testSet[, 1:11], 
                k = trainSetBal %>% nrow() %>% sqrt(), 
                distance = 2, 
                kernel = 'triangluar')
