...
지난 포스팅에서는 Google Map Api를 활용하여 주소 정보로 위경도 좌표를 받고, 해당 좌표를 중심으로 지도를 불러온 후 지도 위에 몇 가지 이미지를 추가하는 방법에 대해 살펴보았습니다. 그 결과로 정적인 지도 이미지를 생성하였습니다만, **leaflet** 패키지를 활용하여 동적 지도 이미지(HTML)를 생성할 수 있습니다.
...
subway$지하철역s <- 
  ...
  str_replace(pattern = '역$', replacement = '') %>% 
  str_c('역')
