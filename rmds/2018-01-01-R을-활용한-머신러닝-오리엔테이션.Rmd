...
## 목차
  ...
  1. R 웹크롤러 (GET, POST, User-agent, JavaScript, RSelenium, Open API)  
  1. 탐색적 데이터 분석 (Explorative Data Analysis)  
  1. 데이터 시각화 : ggpot2, google map API  
  1. 상관분석, t검정, 분산분석  
  1. 차원축소 : 주성분분석 (PCA), 요인분석 (FA), 다차원척도법 (MDS)  
  1. 군집분석 (Clustering) : 계층적 군집분석, K-평균 군집분석  
  1. K근접이웃(KNN)  
  1. 나이브 베이즈(Naive Bayes)  
  1. 의사결정나무 (Decision Tree)  
  1. 단순선형회귀분석 (Linear Regression)  
  1. 다중선형회귀분석 변수선택 (Linear Regression with variable selection)  
  1. 벌점화 회귀모형 : Ridge, LASSO, Elastic Net  
