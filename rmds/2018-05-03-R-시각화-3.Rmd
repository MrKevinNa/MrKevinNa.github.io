...
참고로, 인증키 등록이 완료되지 않았다고 판단하는 근거는 `register_google()` 함수를 실행한 후 `qmap()` 함수로 지도를 불러올 때 **forbidden 403** 에러가 뜬다는 것입니다. 구글 지도 API 서버에서 인증키가 등록되지 않았다는 것이죠. 그러므로 구글 지도 API 서버에서 인증키가 등록되었는지를 판단하려면, RStudio에서 인증키를 등록한 상태에서 `qmap()` 함수를 실행했을 때 정상적으로 지도가 불려와지는 경우를 보고 판단할 수 있습니다. 
