...
화면 아무 곳에서나 오른쪽 클릭을 한 후 "검사(Inspect)"를 선택하면 개발자도구가 열립니다. 개발자도구 맨 상단 메뉴 중 **Network**로 이동합니다. ![](https://github.com/MrKevinNa/MrKevinNa.github.io/blob/master/images/Chrome%20Developer%20Tools%20Image.png?raw=true)
...
이제 새로고침(F5) 합니다. 그러면 뭔가 바쁘게 움직이는 걸 볼 수 있습니다. 하단에 새로 생긴 목록에서 맨 처음 나오는 **search.asp**에 주목합니다. Method는 POST이고 Status는 200입니다. 나머지 항목들은 GET 방식으로 요청되는 것을 확인할 수 있습니다. ![](https://github.com/MrKevinNa/MrKevinNa.github.io/blob/master/images/Chrome%20Developer%20Tools%20Network%200.png?raw=true)

**search.asp**을 클릭하면 우측에 새로운 화면이 열리는데 요청과 반응에 필요한 정보들을 확인할 수 있습니다. - Request URL은 **POST** 함수에 할당할 URL입니다. - Request Method는 "POST"로 되어 있으니 **POST** 방식으로 요청을 해야 합니다. - Status Code는 "200"으로 정상입니다. ![](https://github.com/MrKevinNa/MrKevinNa.github.io/blob/master/images/Chrome%20Developer%20Tools%20Network%201.png?raw=true)

맨 아래로 이동하여 **Form Data**를 확인합니다. POST 함수로 웹페이지 정보를 수집할 때 Form Data를 리스트의 인자로 할당하기 때문에 이 정보들이 필요합니다. ![](https://github.com/MrKevinNa/MrKevinNa.github.io/blob/master/images/Chrome%20Developer%20Tools%20Network%202.png?raw=true)
...
여기서 한 가지 문제가 더 있는데요. (URL 인코딩은 일단 잊어버리고) 각 언어마다 기계가 인식할 수 있도록 인코딩하는 방식이 제각각이었고, 이를 통일하기 위해 국제표준이라 할 수 있는 **UTF-8**가 개발되었습니다. 웹페이지를 개발할 때 인코딩 방식으로 **UTF-8**을 쓰면 좋은데, 한국은 윈도우즈가 많이 사용되어서 그런지 윈도우즈에서 개발한 **EUC-KR** 또는 **CP949**를 사용하는 경우가 많습니다. ![한글 인코딩](https://byjason.github.io/images/posts/character_encoding.png) \* \[참조\] <https://byjason.github.io/2016-08-30/web-scraping-with-r(1)/>
...
**I** 함수는 이중 인코딩(double encoding)을 막기 위해 사용하는 것인데요. httr 패키지는 문자에 대해 URL 인코딩(escape이라고도 함)을 자동으로 해주는데, 문제는 "EUC-KR"인 경우 에러가 발생합니다. 대신에 웹페이지가 "UTF-8" 방식을 사용하고 있는 경우는 **I** 함수를 사용하지 않아도 된다고 합니다. (권경모님 감사합니다. ^^)
...
다음으로 전화번호, 지번주소, 도로명주소를 수집해보겠습니다. 최종 텍스트 앞뒤로 공백이 생길 수 있으므로 **stringr** 패키지의 **str\_trim** 함수를 사용하도록 하겠습니다.
...
sInfo는 10개 상호에 해당하는 정보가 반복되고 있음을 알 수 있습니다. 따라서 데이터의 위치에 따라 다음과 같이 추출하도록 합니다. - 전화번호 : 1, 5, 9와 같이 4로 나누어 나머지가 1인 위치에 있는 데이터 - 지번주소 : 2, 6, 10과 같이 4로 나누어 나머지가 2인 위치에 있는 데이터 - 도로명주소 : 4, 8, 12와 같이 4로 나누어 나머지가 0인 위치에 있는 데이터
...
```
    ...
    ##                                     shopn        phone
    ## 1                   마실 갈치조림추천맛집  02-783-6210
    ## 2                          성민촌추천맛집  02-369-7077
    ## 3                      한양곰치국추천맛집  02-780-7894
    ## 4                        제주나라추천맛집  02-780-3210
    ## 5  다인생굴 모둠꼬치 메로된장구이추천맛집  02-761-9288
    ## 6                            다인추천맛집 070-761-9288
    ## 7                        자린고비추천맛집  02-786-8307
    ## 8                    믹스앤베이크추천맛집  02-780-1252
    ## 9                      한양생태집추천맛집  02-780-5577
    ## 10                 백합구이  해물추천맛집  02-782-3550
    ##                           jibun                          roadn
...
이상으로 **POST** 함수를 이용하여 웹데이터를 수집하는 방법을 알아보았습니다. 다음에는 User-agent를 추가하여 웹크롤러가 아닌 사람이 Request 하는 것처럼 보이는 방법에 대해서 간단하게 알아보겠습니다.
