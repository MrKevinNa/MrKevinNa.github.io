...
웹크롤링을 하다보면 분명히 반응 상태코드가 200으로 정상인데 찾는 tag element가 없는 경우가 있습니다. 이런 경우 **JavaScript**를 의심해볼 수 있습니다 (사실 저는 JavaScript에 대해 1도 모릅니다).

HTML 중간에 `script` tag가 있고 그 아래로 프로그래밍 코드가 보이는 경우가 많은데, 만약 우리가 찾는 tag가 이와 같은 `script`에 영향을 받으면 관련 HTML이 뒤늦게 불려오기 때문에 HTML Response에 찾는 tag가 포함되어 있지 않는 것입니다. (혹시 제가 잘못 기술한 부분이 있다면 피드백을 부탁 드립니다!)
...
15번째 페이지에서 보여지는 것처럼 일부 선수들의 데이터가 `하이픈(-)`로 되어 있습니다. 이것 때문에 숫자가 아닌 문자 데이터로 강제전환된 것입니다. 이것을 제거하고 숫자 데이터로 변환하겠습니다.
