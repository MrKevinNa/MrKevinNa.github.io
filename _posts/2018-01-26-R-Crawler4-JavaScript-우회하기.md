JavaScript가 왜 문제가 되는가?
------------------------------

웹크롤링을 하다보면 분명히 Request 상태코드가 200으로 정상인데 찾는 tag element가 없는 경우가 있습니다. 이런 경우 **JavaScript**를 의심해볼 수 있습니다 (사실 저는 JavaScript에 대해 1도 모릅니다).

HTML 중간에 `script` tag가 있고 그 아래로 프로그래밍 코드들이 보이는 경우가 많은데, 우리가 찾는 tag가 이와 같은 `script`에 영향을 받으면 관련 HTML이 뒤늦게 불려오기 때문에 우리가 받은 HTML에는 포함되어 있지 않는 것입니다. (제가 틀린 부분이 있다면 피드백을 부탁 드립니다!)

위 문제를 해결하는 방법은 2가지가 있습니다.

1.  크롬의 사용자도구에서 '네트워크' 탭을 열고 F5 새로고침을 실행해서 원하는 테이블이 화면에 렌더링되는 시점을 포착하여 관련 항목을 찾는다.
...
데이터 조회 조건으로 아래와 같이 지정하였습니다. - 팀 : "팀-전체" - 포지션 : "포지션-전체" - 시즌범위 : "시즌 시작-2017" - TO : "종료 시즌-2017" - 정규/포스트 시즌 구분 : "정규시즌" - 분류1 : "분류-선택안함" - 타석수 : "타석수-전체"

위와 같이 설정한 후 우측에 있는 "결과" 버튼을 클릭하면 아래에 테이블 형태로 데이터가 출력됩니다. 출력된 상태에서 크롬 개발자도구를 열고 **Network** 탭으로 이동합니다. 그리고 새로고침(F5)을 누릅니다. 매우 많은 항목들이 주르륵 생기는데, 이 때 주목할 것은 개발자도구 위에서 세 번째 줄에 있는 메뉴입니다. 아마도 **All**이 선택되어 있을 것입니다. 이것을 **XHR**로 변경합니다. 그럼 항목이 크게 줄어듭니다.

**XHR**은 (제가 1도 몰라서 찾아보니) **XML Http Request**의 머릿글자로 **AJAX** 요청을 생성하는 **JavaScript API**라고 합니다[1]. 그리고 **AJAX**는 **Asynchronos Javascript And XML**의 머릿글자로 JavaScript와 XML을 의미한다고 합니다. 단순하게 요약하자면, **JavaScript**를 통해서 웹서버로부터 **XML** 데이터를 요청하는 것입니다[2].

저도 잘 모르는 걸 설명하려니 어렵지만, 다시 크롬 개발자도구로 돌아가서 (봉사가 문고리를 잡고, 소가 뒷걸음치다가 쥐를 잡듯이) **XHR**로 이동하면 첫 번째 그림처럼 보일 것입니다.

![](https://github.com/MrKevinNa/MrKevinNa.github.io/blob/master/images/Chrome%20Developer%20Tools%20Network%206.png?raw=true)

이 때 새로고침을 하면 아래와 같이 두 번째 그림처럼 몇 가지 항목이 생성되는데 중간에 **AJAX**가 있고 **POST()** 방식으로 요청한다는 것을 확인할 수 있습니다.
...
위와 같이 하면 아주 간단하게 원하는 데이터를 수집할 수 있습니다. 그런데 지금 우리는 겨우 20명의 타자 데이터만 수집하였습니다. 화면 맨 아래에 페이지를 이동하는 부분이 있습니다. 맨 끝 페이지로 이동해보니 총 15 페이지가 있음을 확인할 수 있었습니다.

페이지 이동은 어떻게 하면 좋을까요? 왠지 앞에서 `POST()`의 `body`인자를 설정할 때 `page = 1`로 되어 있던 기억이 납니다. 그럼 여기에서 1 대신에 2를 넣으면 2페이지가 되겠죠? 즉, page에 할당되는 값을 1부터 15까지 순환하며 데이터를 수집하면 됩니다.
...
모든 데이터를 다 수집하였으니 이제 데이터 정제과정을 거치겠습니다. 거의 모든 데이터가 숫자 데이터이므로 번거로운 작업은 많지 않을 것 같습니다.
...
15번째 페이지에서 보여지는 것처럼 일부 선수들의 데이터가 `하이픈(-)`로 되어 있습니다. 이것 때문에 숫자가 아닌 문자 데이터로 강제전환된 것입니다. 이것을 제거하고 숫자 데이터로 변환해주겠습니다.
...
다음에는 이번에 저장한 데이터를 가지고 여러 가지 그래프를 그려봄으로써 다양한 인사이트를 얻는 탐색적 데이터 분석(EDA)에 대해서 정리해보겠습니다.

[1] 자세한 내용은 [여기](https://developer.mozilla.org/ko/docs/Glossary/XHR_(XMLHttpRequest))를 확인해보시기 바랍니다

[2] 이 부분은 [여기](http://wherethelightis.tistory.com/14)에서 잘 설명되어 있습니다
